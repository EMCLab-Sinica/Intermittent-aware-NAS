...
class StateSpace:
    ...
    def print_actions(self, actions):
        ...
        print('NASBase::CONTROLLER:: Actions :')
...
class Controller:
    ...
    '''
    def __init__(self, inas_settings, policy_session, num_layers, state_space,
                 reg_param=0.001,
        ...
        self.global_exp_suffix = inas_settings.GLOBAL_SETTINGS["EXP_SUFFIX"]
        ...
        print("NASBase::CONTROLLER:: state_size=", self.state_size)
    ...
    def get_rand_action(self, state):
        print("NASBase::CONTROLLER:: Generating random action to explore")
        actions = []
    ...
    def get_action(self, state):
        ...
        if np.random.random() < self.exploration:
            ...
            print("NASBase::CONTROLLER:: Prediction action from Controller")
            ...
            print("NASBase::CONTROLLER:: State input to Controller for Action : ", state.flatten())
    ...
    def build_policy_network(self):
        with self.policy_session.as_default():
            ...
            with tf.name_scope('losses'):
                ...
                tf.summary.scalar('regularization', self.reg_strength * reg_loss)
            ...
            self.summary_writer = tf.summary.FileWriter(filename, graph=self.policy_session.graph, filename_suffix=self.global_exp_suffix)

            self.policy_session.run(tf.global_variables_initializer())
            self.saver = tf.train.Saver(max_to_keep=1)

            if self.restore_controller:
                path = tf.train.latest_checkpoint('weights/' + self.global_exp_suffix + "/")

                if path is not None and tf.train.checkpoint_exists(path):
                    print("NASBase::CONTROLLER:: Loading Controller Checkpoint !")
    ...
    def store_rollout(self, state, reward):
        ...
        if len(self.reward_buffer) > 20:
            with open('tmp_buffers/buffers_' + self.global_exp_suffix + '.txt', mode='a+') as f:
                for i in range(20):
                ...
                print("NASBase::CONTROLLER:: Saved buffers to file `buffers.txt` !")
    ...
    def train_step(self):
        ...
        print("NASBase::CONTROLLER:: State input to Controller for training : ", state_input.flatten())
        ...
        with self.policy_session.as_default():
            ...
            print("NASBase::CONTROLLER:: Training RNN (States ip) : ", state_list)
            print("NASBase::CONTROLLER:: Training RNN (Reward ip) : ", reward.flatten())
            _, loss, summary, global_step = self.policy_session.run([self.train_op, self.total_loss, self.summaries_op,
                                                                     self.global_step],
                                                                     feed_dict=feed_dict)

            self.summary_writer.add_summary(summary, global_step)
            self.saver.save(self.policy_session, save_path='weights/' + self.global_exp_suffix + '/controller.ckpt', global_step=self.global_step)
